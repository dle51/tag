...
class Chase(RobotEnv):
    ...
    def look_at_each_other(self):
        self.robot.robots.robots[1].robot.robot.set_quat([0.7071, 0.0, 0.0, 0.7071])
        self.robot.robots.robots[2].robot.robot.set_quat([0.7071, 0.0, 0.0, -0.7071])
        return

    def check_for_collisions(self):
        contacts = self.robot.robots.robots[1].robot.robot.get_contacts(
            with_entity=self.robot.robots.robots[2].robot.robot
        )["geom_a"]
        return torch.any(contacts != 0)

    def reset(self):
        obs, _ = super().reset()
        self.look_at_each_other()
        return obs, _
